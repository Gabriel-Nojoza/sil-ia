...
function mapCompanyRow(row: CompanyRow | LegacyCompanyRow): CompanyRecord {
  return {
    ...
    powerbiIdentityMode: row.powerbi_identity_mode ?? "service_principal",
...
function mapCompanyInput(input: CompanyBaseInput): CompanyInsertRow {
  return {
    ...
    timezone: nullIfEmpty(input.timezone) ?? "America/Sao_Paulo",
    language: nullIfEmpty(input.language) ?? "pt-BR",
    powerbi_identity_mode: input.powerbiIdentityMode ?? "service_principal",
